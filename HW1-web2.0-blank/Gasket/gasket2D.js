...
window.onload = function init()
{
	...
	canvas.addEventListener("mousedown", function(event){
		...
		//--------------------------------------------------------------------------------
	    centerX= (x-rect.left)/(rect.right-rect.left)*2-1;
	    centerY= (rect.bottom-y)/(rect.bottom-rect.top)*2-1;
...
function  countFPS() {
    ...
    if (now > 1000 + lastTime) {
		...
		document.getElementById("label").innerHTML=("FPS: "+fps+" 分形图形的顶点数: "+points.length);
        frame = 0;
        lastTime = now;
    };
